...
def test_put_an_object(update_object_endpoint, new_object_id):
    ...
    update_object_endpoint.make_changes_in_object(new_object_id, payload_to_test)
...
def test_patch_an_object(update_object_endpoint, new_object_id):
    payload_to_test = {"data": {"color": "new", "size": "PATCH"}}
    update_object_endpoint.patch_object(new_object_id, payload_to_test)
    update_object_endpoint.check_status_is_200()
    update_object_endpoint.check_response_color_is_correct(payload_to_test["data"]["color"])
